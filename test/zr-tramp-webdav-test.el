...
(ert-deftest zr-tramp-webdav-background-cache-misses-are-quiet ()
  (let* ((root "/webdav:background.example.invalid:/dav/")
         (file (concat root "note.txt"))
         (tramp-cache-data (make-hash-table :test #'equal))
         (tramp-verbose 0)
         (auth-sources nil)
         (non-essential t)
         ...
         (debug-messages 0)
         results)
    (cl-letf (((symbol-function 'zr-tramp-webdav--curl-request)
               (lambda (&rest _args)
                 (cl-incf requests)
                 '(:status 404)))
              ((symbol-function 'zr-tramp-webdav--url-request)
               (lambda (&rest _args)
                 (cl-incf requests)
                 '(:status 404)))
              ((symbol-function 'tramp-debug-message)
               (lambda (&rest _args) (cl-incf debug-messages))))
      (dolist (call `((file-attributes ,file)
                      (file-exists-p ,file)
                      (file-directory-p ,root)
                      (directory-files ,root)
                      (directory-files-and-attributes ,root)
                      (file-name-all-completions "" ,root)
                      (file-name-completion "n" ,root)))
        (push (condition-case err
                  (apply (car call) (cdr call))
                (file-error err))
              results)))
    (should (= requests 0))
    ;; TRAMP used to force level-10 backtraces even with verbosity zero.
    (should (= debug-messages 0))
    (should (cl-every #'null results))
    ;; Skipping a request must not cache a false "missing" result.
    (with-parsed-tramp-file-name file nil
      (should (eq (tramp-get-file-property v localname "zr-webdav-stat"
                                          tramp-cache-undefined)
                  tramp-cache-undefined)))
    (with-parsed-tramp-file-name root nil
      (should (eq (tramp-get-file-property v localname "directory-webdav"
                                          tramp-cache-undefined)
                  tramp-cache-undefined)))))
...
(ert-deftest zr-tramp-webdav-background-checks-use-cached-listings ()
  (zr-tramp-webdav-test--with-backends
    (let ((file (concat root "note.txt"))
      ...
      (make-directory directory)
      (zr-tramp-webdav-clear-cache)
      (let* ((transport (if (eq zr-tramp-webdav-backend 'curl)
                            'zr-tramp-webdav--curl-request
                          'zr-tramp-webdav--url-request))
             (original (symbol-function transport))
             (requests 0))
        (cl-letf (((symbol-function transport)
          ...
          (let ((non-essential t))
            ...
            (should-not (file-name-all-completions "" root)))
          (should (= requests 0))
